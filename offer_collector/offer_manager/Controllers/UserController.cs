...
namespace offer_manager.Controllers
{
    ...
    [Route("api/users")]
    [ApiController]
    public partial class UserController(User userController, UserPasswordPolicy userPasswordPolicy, JwtOptions jwtOptions, Logger logger) : ControllerBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Application logger used by <see cref="UserController"/> to record diagnostic and audit-style events.
        /// Uses the custom <see cref="Logger"/> (not Microsoft.Extensions.Logging).
        /// Logging must never affect request processing; all logging calls are wrapped with safeguards.
        /// </summary>
        private readonly Logger _logger = logger;

        /// <summary>
        /// Attempts to write a log entry using the custom <see cref="Logger"/>.
        /// The log is tagged with controller and action context: <c>["UserController", action]</c>.
        /// Any exceptions thrown by the logger are swallowed to ensure logging never breaks an HTTP request.
        /// </summary>
        /// <param name="type">
        /// Log severity/type (e.g. <c>"INFO"</c>, <c>"WARNING"</c>, <c>"ERROR"</c>, <c>"DEBUG"</c>, <c>"NOTIFICATION"</c>).
        /// </param>
        /// <param name="action">
        /// Logical action name (typically the endpoint/method name) used as part of the log context.
        /// </param>
        /// <param name="message">Human-readable log message.</param>
        private void TryLog(string type, string action, string message)
        {
            try
            {
                _logger.Add(type, new[] { "UserController", action }, message);
            }
            catch
            {
        ...
        /// <summary>
        /// Helper that logs an exception as an <c>"ERROR"</c> entry for the given action.
        /// The message format is: <c>{action}: {ExceptionType}: {ExceptionMessage}</c>.
        /// This method is safe to call from catch blocks; it never throws.
        /// </summary>
        /// <param name="action">
        /// Logical action name (typically the endpoint/method name) to associate the exception with.
        /// </param>
        /// <param name="ex">The exception instance to be logged.</param>
        private void TryLogException(string action, Exception ex)
        {
            TryLog("ERROR", action, $"{action}: {ex.GetType().Name}: {ex.Message}");
        }
        ...
        {
            const string action = "standard-register";
            TryLog("INFO", action, "Request received.");
            if (request is null)
            {
            ...
            try
            {
                ...
                if (!result)
                {
                    TryLog("ATTENTION", action, "Conflict.");
                ...
                TryLog("NOTIFICATION", action, "Created.");
                return StatusCode(StatusCodes.Status201Created, new StandardRegisterUserResponse { ErrorMessage = string.Empty });
            }
            catch (Exception ex)
            {
                TryLogException(action, ex);
                TryLog("ERROR", action, "Internal server error response.");
        ...
        {
            const string action = "standard-login";
            TryLog("INFO", action, "Request received.");
            if (request is null)
            {
                TryLog("WARNING", action, "BadRequest.");
            ...
            try
            {
                ...
                if (!result)
                {
                    TryLog("ATTENTION", action, "Unauthorized.");
                ...
                TryLog("NOTIFICATION", action, "Success.");
        ...
        {
            const string action = "token-login";
            TryLog("INFO", action, "Request received.");
            if (request is null)
            {
                TryLog("WARNING", action, "BadRequest.");
        ...
        {
            const string action = "update-data";
            TryLog("INFO", action, "Request received.");
            if (request is null)
            {
                TryLog("WARNING", action, "BadRequest.");
            ...
            {
                TryLog("ATTENTION", action, "JWT expired.");
                TryLog("ATTENTION", action, "Unauthorized.");
        ...
        {
            const string action = "update-weights";
            TryLog("INFO", action, "Request received.");
            if (request is null)
            {
                TryLog("WARNING", action, "BadRequest.");
        ...
        {
            const string action = "change-password";
            TryLog("INFO", action, "Request received.");
            if (request is null)
            {
                TryLog("WARNING", action, "BadRequest.");
        ...
        {
            const string action = "check-permission";
            TryLog("INFO", action, "Request received.");
            if (request is null)
            {
                TryLog("WARNING", action, "BadRequest.");
            ...
            try
            {
                ...
                if (!hasPermission)
                {
                    TryLog("ATTENTION", action, "Forbidden.");
        ...
        {
            const string action = "insert-search-history";
            TryLog("INFO", action, "Request received.");
            if (request is null)
            {
                TryLog("WARNING", action, "BadRequest.");
        ...
        {
            const string action = "delete-user";
            TryLog("INFO", action, "Request received.");
            if (request is null)
            {
                TryLog("WARNING", action, "BadRequest.");
        ...
        {
            const string action = "refresh-jwt";
            TryLog("INFO", action, "Request received.");
            if (request is null)
            {
                TryLog("WARNING", action, "BadRequest.");
        ...
        /// </returns>
        [HttpPost("check-jwt")]
        public async Task<ActionResult<CheckJwtResponse>> CheckJwt(
            [FromBody] CheckJwtRequest? request
        )
        {
            const string action = "check-jwt";
            TryLog("INFO", action, "Request received.");
            if (request is null)
            {
                TryLog("WARNING", action, "BadRequest.");
        ...
        {
            const string action = "get-search-history";
            TryLog("INFO", action, "Request received.");
            if (request is null)
            {
                TryLog("WARNING", action, "BadRequest.");
        ...
        {
            const string action = "get-weights";
            TryLog("INFO", action, "Request received.");
            if (request is null)
            {
                TryLog("WARNING", action, "BadRequest.");
        ...
        {
            const string action = "update-preferences";
            TryLog("INFO", action, "Request received.");
            if (request is null)
            {
                TryLog("WARNING", action, "BadRequest.");
        ...
        {
            const string action = "get-preferences";
            TryLog("INFO", action, "Request received.");
            if (request is null)
            {
                TryLog("WARNING", action, "BadRequest.");
            ...
            try
            {
                ...
                if (isIncomplete)
                {
                    TryLog("WARNING", action, "Unprocessable entity.");
        ...
        /// <summary>
        /// Returns the most recent N stored searches (globally, across all users).
        /// For full request/response details and examples, see <c>UserAPI-DOCS.md</c>.
        /// </summary>
        /// <param name="request">Request body containing the limit (N).</param>
        /// <returns>
        /// 200 with payload; 400 on invalid input; 500 on server error.
        /// </returns>
        [HttpPost("get-last-searches")]
        public async Task<ActionResult<GetLastSearchesResponse>> GetLastSearches(
            [FromBody] GetLastSearchesRequest? request
        )
        {
            const string action = "get-last-searches";
            TryLog("INFO", action, "Request received.");

            if (request is null)
            {
                TryLog("WARNING", action, "BadRequest. Empty request.");
                return BadRequest(new GetLastSearchesResponse
                {
                    ErrorMessage = _errEmptyRequest,
                    LastSearches = JsonDocument.Parse("[]").RootElement.Clone()
                });
            }

            try
            {
                TryLog("DEBUG", action, $"Fetching last searches. Limit={request.Limit}.");
                string json = await _userController.GetLastSearchesAsync(request.Limit, HttpContext.RequestAborted);
                using JsonDocument doc = JsonDocument.Parse(json);

                TryLog("NOTIFICATION", action, "Ok.");
                return Ok(new GetLastSearchesResponse
                {
                    ErrorMessage = string.Empty,
                    LastSearches = doc.RootElement.Clone()
                });
            }
            catch (Exception ex)
            {
                TryLogException(action, ex);
                TryLog("ERROR", action, "Internal server error response.");
                return StatusCode(StatusCodes.Status500InternalServerError, new GetLastSearchesResponse
                {
                    ErrorMessage = _errInternalServer,
                    LastSearches = JsonDocument.Parse("[]").RootElement.Clone()
                });
