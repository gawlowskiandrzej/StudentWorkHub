...
namespace Offer_collector
{
    internal class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            if (args.Length < 2)
            {
                Console.WriteLine("Użycie: App.exe <siteTypeId> <offerAmount> <clientType>");
                Console.WriteLine("Przykład: App.exe 1 100 1");
                return;
            }
            ...
            await foreach (var (offersJson, htmls, errors) in paginator.FetchAllOffersAsync(searchFilters, bathSize: bathSize))
            {
                List<UnifiedOfferSchemaClass> unifiedOfferSchemastemp = new List<UnifiedOfferSchemaClass>();
                switch (type)
                {
                    case OfferSitesTypes.Pracujpl:
                        List<PracujplSchema> pracujSchemas = OfferMapper.DeserializeJson<List<PracujplSchema>>(offersJson);
                        ...
                        foreach (PracujplSchema offer in pracujSchemas)
                        {
                            ...
                            unifiedOfferSchemastemp.Add(unifOffer);
                        }

                        //ExportToTxtt.ExportToTxt(output, "pracujData.txt");
                        break;
                    case OfferSitesTypes.Olxpraca:
                        List<OlxPracaSchema> olxSchemas = OfferMapper.DeserializeJson<List<OlxPracaSchema>>(offersJson);
                        ...
                        foreach (OlxPracaSchema offer in olxSchemas)
                        {
                            ...
                            unifiedOfferSchemastemp.Add(olxSch);
                        }
                        break;
                    case OfferSitesTypes.Justjoinit:
                        List<JustJoinItSchema> justJoinItSchemas = OfferMapper.DeserializeJson<List<JustJoinItSchema>>(offersJson);
                        ...
                        break;
                    case OfferSitesTypes.Aplikujpl:
                        List<AplikujplSchema> aplikujSchemas = OfferMapper.DeserializeJson<List<AplikujplSchema>>(offersJson);
                ...
                // Processed by Ai
                (List<string> aioffers, List<string> errormessages) processedOffers = await GetAiOutput(aiParser, unifiedOfferSchemastemp, databaseService, bathSize);
                errrosAll.AddRange(processedOffers.errormessages);
                ExportTo.ExportToJs(errrosAll, $"{Enum.GetName(typeof(OfferSitesTypes), siteTypeId)}-errors.json");
        ...
        static async Task<(List<string>, List<string>)> GetAiOutput(AIProcessor llm, List<UnifiedOfferSchemaClass> offers, DBService service, int bathSize) => await llm.ProcessUnifiedSchemas(offers, service, bathSize);
    }
}