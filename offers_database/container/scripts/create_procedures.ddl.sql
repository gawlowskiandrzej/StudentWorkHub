...
BEGIN
  ...
  INSERT INTO public.companies(name, logo_url) VALUES (p_company_name, p_company_logo_url)
  ON CONFLICT (name) DO NOTHING;
  SELECT id INTO v_company_id FROM public.companies WHERE name = p_company_name;
  IF p_company_logo_url IS NOT NULL THEN
    UPDATE public.companies SET logo_url = p_company_logo_url WHERE id = v_company_id AND logo_url IS NULL;
  END IF;
