...
export function FloatingLabelInput({ type = "text", label, value, className ,onChange }: FloatingLabelInputProps) {

  return (
    <div className={`relative w-full ${className}`}>
    <input value={value} type={type} onChange={onChange} id={`floating_outlined${label}`} className={`${buttonStyle["floating-input"]} block px-2.5 pb-2.5 pt-4 w-full text-sm text-heading bg-background rounded-base border-1 border-default-medium appearance-none focus:outline-none focus:ring-0 focus:border-brand peer`} placeholder=" "/>
