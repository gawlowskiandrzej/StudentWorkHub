...
export const UserProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const isJwtExpired = useMemo(() => {
    if (!jwt) return true;
    if (!jwtExpiresAtMs) return false; // unknown -> assume not expired here
    return Date.now() >= jwtExpiresAtMs;
  }, [jwt, jwtExpiresAtMs]);

  const isJwtExpiringSoon = useMemo(() => {
    if (!jwt) return false;
    if (!jwtExpiresAtMs) return false;
    return jwtExpiresAtMs - Date.now() <= JWT_REFRESH_SAFETY_WINDOW_MS;
  }, [jwt, jwtExpiresAtMs]);
