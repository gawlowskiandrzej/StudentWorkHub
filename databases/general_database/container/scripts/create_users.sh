...
BEGIN
    ...
    ELSE
        EXECUTE format('ALTER ROLE %I WITH LOGIN PASSWORD %L', '${BACKUP_DB_USER}', '${BACKUP_DB_PASSWORD}');
    END IF;
...
-- Allow both users to connect
GRANT CONNECT ON DATABASE "$POSTGRES_DB" TO "${BACKUP_DB_USER}";
GRANT CONNECT ON DATABASE "$POSTGRES_DB" TO "${APP_DB_USER}";

-- Allow both to use public schema (but not create objects)
GRANT USAGE ON SCHEMA public TO "${BACKUP_DB_USER}";
GRANT USAGE ON SCHEMA public TO "${APP_DB_USER}";

-- BACKUP USER: read-all for pg_dump (logical backups)
-- (This is strong read access to all tables across DB; still not superuser.)
